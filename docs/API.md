...
3. 눈덩이 생성:
   ...
   - 속도: 팀에 따라 상대 진영 방향으로 대각선 발사 (맵은 `\` 대각선으로 분할)
     - Red팀 (우상단, y<=x): (-4, +4) - 좌하단 방향 (Blue 진영으로)
     - Blue팀 (좌하단, y>=x): (+4, -4) - 우상단 방향 (Red 진영으로)
...
- 눈덩이 충돌 시 피해 적용 중단
- 봇 AI 동작 중단
- 3초 후 게임 루프 정지 및 모든 봇 제거

클라이언트 측:
- 승리/패배/무승부 메시지 표시
- "Return to Menu" 버튼 표시
- 승리 팀: 이동 가능, 정지 시 만세 애니메이션 재생
- 패배 팀: 모든 입력 차단
- 모든 플레이어: 눈덩이 투척 불가
- 에너지 바 숨김
...
room.onMessage('gameEnded', (message) => {
  ...
  // 클라이언트는 게임 종료 화면을 표시하고
  // 승리 팀에게 만세 애니메이션과 이동 권한을 부여합니다
});
```
...
- 봇은 1초마다 랜덤 방향으로 이동하며 (플레이어와 동일한 속도 2) 2초마다 눈덩이를 발사합니다 (일반 데미지 4)
