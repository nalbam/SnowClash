...
| 메서드 | 시점 | 역할 |
|--------|------|------|
| `onCreate` | 방 생성 시 | 상태 초기화, 메시지 핸들러 등록 |
...
// 게임 시작 (초기 위치 배치, 게임 루프 시작)
private startGame(): void

// 게임 루프 (60 FPS) - 눈덩이 업데이트, 충돌 검사
...
class PlayerSchema extends Schema {
  ...
  @type('string') googleId    // Google OAuth ID
  @type('string') photoUrl    // 프로필 사진 URL
...
room.state.players.onAdd((player, sessionId) => {
  ...
  // 개별 플레이어 변경 감지
  player.onChange(() => {
    updatePlayerGraphics(sessionId, player);
  });
});
...
// 눈덩이도 동일한 패턴
room.state.snowballs.onAdd(...);
room.state.snowballs.onRemove(...);
```
