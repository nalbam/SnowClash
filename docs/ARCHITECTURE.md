...
| 버전 표시 | 클라이언트 버전(config.clientVersion) 및 서버 버전(GET /api/version)을 화면 하단에 표시 |
...
| **버전 정보** | 화면 하단 (y: MAP_SIZE - 20), 10px, 회색 (#999999), 형식: "Client: v{version} \| Server: v{version}" |
...
│   └────────────────────────────┘     │
│                                      │
│ Client: v1.1.0 | Server: v1.1.0      │  ← 버전 정보
└──────────────────────────────────────┘
```
...
| 모바일 컨트롤 | 가상 조이스틱 (좌하단) + 가상 공격 버튼 (우하단) |
| 차징 게이지 | 좌하단 (10, 560), 100x20px, 노란색 → 빨간색 (데스크톱) / 공격 버튼 주변 (모바일) |
...
GameScene은 모듈화된 시스템들을 사용하여 게임 로직을 구현합니다:

| 시스템 | 역할 |
|--------|------|
| **InputSystem** | 키보드, 마우스, 터치 입력 처리 및 차징 관리 |
| **PlayerRenderSystem** | 플레이어 스프라이트 렌더링, 애니메이션, UI 요소 (닉네임, 에너지 바) |
| **SnowballSystem** | 눈덩이 렌더링 및 충돌 시 파편 효과 |
| **VirtualControllerSystem** | 모바일 전용 가상 조이스틱 및 공격 버튼 (터치 디바이스 자동 감지) |
...
| S / ↓ | 아래로 이동 |
| D / → | 오른쪽으로 이동 |
| **마우스/터치 클릭** | 클릭한 위치로 이동 (360° 자유 방향, 데스크톱) |
| **가상 조이스틱** | 360° 아날로그 이동 제어 (모바일, 좌하단) |
| Space (탭) | 일반 공격 (최소 0.2초 차징 필요) |
| Space (홀드 0.7초+) | 차징 공격 (7 데미지) |
| **마우스/터치 홀드** | 차징 (키보드와 동일하게 작동, 데스크톱) |
| **가상 공격 버튼** | 차징 공격 (모바일, 우하단, 시각적 차징 게이지 포함) |
...
SnowClash는 **팀별 시점 회전(Team-specific Perspective Rotation)** 시스템을 사용하여 각 팀이 자신의 진영을 화면 하단에서 보도록 합니다.

### 기본 원리

| 팀 | 시점 | 좌표 변환 |
|----|------|----------|
| **Blue팀** | 기본 시점 (0°) | 변환 없음 `(x, y)` |
| **Red팀** | 180° 회전 시점 | 대칭 변환 `(MAP_SIZE - x, MAP_SIZE - y)` |

**효과**: Red 팀 플레이어는 맵이 180도 회전되어 보이므로, 자신의 진영(서버 좌표상 우상단)이 화면 하단에 표시됩니다.

### 변환 메서드

모든 클라이언트 씬과 시스템은 다음 두 메서드를 구현합니다:
...
| 컴포넌트 | 메서드 | 용도 |
|---------|--------|------|
| **GameScene** | `toViewCoords`, `toServerCoords` | 렌더링 및 클라이언트 예측 |
| **InputSystem** | `toServerCoords` | 포인터/터치 입력 변환, 키보드 방향 반전 |
| **PlayerRenderSystem** | `toViewCoords` | 플레이어 스프라이트 위치 변환 |
| **SnowballSystem** | `toViewCoords` | 눈덩이 및 파편 효과 위치 변환 |
| **LobbyScene** | `toViewCoords`, `toServerCoords` | 팀 선택 및 로비 플레이어 배치 |
...
└────────────┬────────────┘
             ...
             ▼
    ┌────────────────────┐
    │ toServerCoords()   │ ← InputSystem이 적용
    │ (red면 180° 회전)  │
    └────────┬───────────┘
             │
             ▼
...
**1. InputSystem (입력 처리)**
- 포인터 클릭: `toServerCoords(pointer.x, pointer.y)` → 서버 좌표로 변환
- 키보드 입력: Red 팀은 방향 반전 (`moveX = -input.moveX`)

**2. PlayerRenderSystem (플레이어 렌더링)**
- 플레이어 스프라이트 위치: `toViewCoords(player.x, player.y)`
- 닉네임, 에너지 바 위치도 함께 변환

**3. SnowballSystem (눈덩이 렌더링)**
- 눈덩이 위치: `toViewCoords(snowball.x, snowball.y)`
- 충돌 효과 파편: `toViewCoords(snowball.x, snowball.y)`

**4. LobbyScene (로비)**
- 팀 선택 클릭: `toServerCoords(click.x, click.y)` → 어느 팀 영역인지 판단
- 플레이어 아바타 배치: `toViewCoords(lobbyPos.x, lobbyPos.y)`

### 장점

1. **직관적인 플레이**: 모든 플레이어가 자신의 진영을 화면 하단에서 볼 수 있음
2. **공정성**: 양 팀 모두 동일한 시점 경험
3. **단순한 서버 로직**: 서버는 하나의 좌표계만 관리
4. **대칭적 변환**: `toViewCoords`와 `toServerCoords`가 동일한 공식 사용 (역변환 = 정변환)
...
│   │   │   └── VirtualControllerSystem.ts # 모바일 가상 컨트롤러 (조이스틱, 공격 버튼)
│   │   └── scenes/
