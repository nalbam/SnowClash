...
### 팀별 시점 (Team Perspective)

각 팀은 **자신의 진영이 화면 하단에 보이도록** 회전된 시점으로 게임을 플레이합니다.

| 팀 | 시점 | 설명 |
|----|------|------|
| **Blue팀** | 기본 시점 (0°) | 좌하단 영역이 화면 하단에 표시 (자기 진영) |
| **Red팀** | 180° 회전 시점 | 우상단 영역이 화면 하단에 표시 (자기 진영) |

**효과**:
- 모든 플레이어가 자신의 진영에서 상대 진영을 향해 공격하는 경험
- 양 팀 모두 동일한 플레이 경험과 공정성 제공
- 서버는 단일 좌표계로 관리 (Blue 팀 기준)

**좌표 변환**:
- Red 팀 플레이어의 화면 입력은 `(MAP_SIZE - x, MAP_SIZE - y)` 변환 후 서버로 전송
- 서버 상태는 Red 팀 화면에 렌더링 시 180° 회전되어 표시

상세한 구현은 [Architecture Guide](./ARCHITECTURE.md#좌표-변환-시스템)를 참조하세요.
...
| **키보드** | S / ↓ | 아래로 이동 (y + 1) |
| **키보드** | D / → | 오른쪽으로 이동 (x + 1) |
| **마우스** | 클릭 앤 홀드 | 커서 위치를 향해 이동 (데스크톱) |
| **터치 (데스크톱)** | 터치 앤 홀드 | 터치 위치를 향해 이동 |
| **가상 조이스틱 (모바일)** | 드래그 | 360° 아날로그 이동 제어 (좌하단) |

**포인터 기반 이동 (마우스/터치 - 데스크톱)**
...
**가상 조이스틱 (모바일)**
- 화면 좌하단에 위치한 가상 조이스틱
- 드래그하여 360° 아날로그 방향 제어
- 시각적 피드백: 조이스틱 베이스와 썸(thumb)
- 조이스틱 반경: 50px, 썸 반경: 25px
- 터치 디바이스 자동 감지 시 활성화
...
| **마우스** | 클릭 앤 홀드 (홀드하여 차징, 놓으면 발사, 데스크톱) |
| **터치 (데스크톱)** | 탭 앤 홀드 (홀드하여 차징, 놓으면 발사) |
| **가상 공격 버튼 (모바일)** | 버튼 탭/홀드 (차징 게이지 포함, 우하단) |
...
**가상 공격 버튼 (모바일)**
- 화면 우하단에 위치한 원형 공격 버튼
- 탭하여 일반 공격, 홀드하여 차징 공격
- 시각적 차징 게이지: 버튼 주변에 원형으로 표시
  - 노란색: 0~0.7초 차징 (일반 데미지)
  - 빨간색: 0.7초+ 차징 (차징 데미지)
- 버튼 반경: 45px
- 터치 디바이스 자동 감지 시 활성화
...
데스크톱에서 포인터(마우스/터치)는 이동과 공격을 모두 처리합니다:
